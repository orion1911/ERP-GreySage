...
import { ContentCut as ContentCutIcon, Edit as EditIcon, Delete as DeleteIcon, Check as CheckIcon, SwapVert } from '@mui/icons-material';
...
function StitchingVendorCatalog() {
  ...
  const getStitchingVendors = () => {
    setLoading(true);
    apiService.stitchingVendors.getStitchingVendors(search, showInactive)
      .then(res => {
        setTimeout(() => {
          setVendors(res);
          setLoading(false);
  ...
  const handleSaveOrder = (orderedIds) => {
    setSavingOrder(true);
    apiService.stitchingVendors.reorderStitchingVendors(orderedIds)
      .then(() => {
