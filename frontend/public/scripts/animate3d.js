...
function App(conf) {
    ...
    function updateColors() {
        ...
        for (let i = 0; i < objects.length; i++) {
            ...
            objects[i].targetColor = targetColor; // store target color
        }

        // Smoothly change light colors too
    ...
    function animate() {
        ...
        animateObjects();
        animateLights();
        // Smoothly transition tube colors
        for (let i = 0; i < objects.length; i++) {
            if (objects[i].targetColor) {
                objects[i].material.color.lerp(objects[i].targetColor, 0.05); // 0.05 controls speed
            }
        }

        // Smoothly transition light colors
