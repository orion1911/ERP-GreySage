...
function FabricVendorCatalogAdd({ open, onClose, loading, setLoading, onAddSuccess, editVendor }) {
  ...
  const onSubmit = (data) => {
    ...
    const request = editVendor
      ? apiService.fabricVendors.updateFabricVendor(editVendor._id, data)
      ...
      >
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h6" id="add-vendor-modal">
            {editVendor ? 'Edit Fabric Vendor' : 'Add Fabric Vendor'}
          </Typography>
